		...
		<div class="mx-10 flex flex-col gap-5 pb-10 pt-10 sm:mx-3 sm:flex-wrap">
			...
			<div class="transparent-800 flex w-full flex-row flex-wrap items-center justify-between rounded-xl p-8 text-left sm:p-4">
				<div class="flex flex-col items-start gap-3">
					...
					<h3>You can wake up other devices on your network or you can add other computers running Cores.</h3>
				</div>

				<div class="flex flex-col items-start gap-3 sm:my-5">
					<ModularDialog title={"Add device"} description={"You can get your connection code from the Cores desktop app."}>
					...
					{#each $settings.connectionCodes.filter((item) => item.code !== $settings.connectionCode) as item, i}
						<div class="flex w-full flex-row flex-wrap items-center justify-between gap-3">
							...
							<div class="flex flex-row flex-wrap gap-3">
								...
								<ModularDialog title={"Edit device"} description={"You can get your connection code from the Cores desktop app."}>
									<slot slot="openButton">
										<Dialog.Trigger class="button mt-6">
											<Pencil />
											Edit
										</Dialog.Trigger>
									</slot>
...
<script lang="ts">
	import { Clipboard, ExternalLink, MonitorSmartphone, KeyRound, Network, Plus, Power, Trash2, Pencil } from "lucide-svelte"

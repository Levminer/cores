		...
		<div class="flex w-full flex-row items-start justify-start gap-5 sm:flex-wrap">
			...
			<div class="flex w-3/5 flex-col gap-5 sm:w-full">
				...
				{#if $hardwareInfo.ram.info.length > 0}
					<div class="transparent-800 rounded-xl p-8 sm:w-full sm:p-4">
								...
								statistics: [
									{
										...
										color: "max",
										data: minutes
											? $hardwareStatistics.minutes.map((value) => value.ram.temperature!.max)
											: $hardwareStatistics.seconds.map((value) => value.ram.temperature!.max),
									},
									{
										label: "Current Temperature",
										color: "current",
										data: minutes
											? $hardwareStatistics.minutes.map((value) => value.ram.temperature!.value)
											: $hardwareStatistics.seconds.map((value) => value.ram.temperature!.value),
									},
									{
										label: "Min Temperature",
										color: "min",
										data: minutes
											? $hardwareStatistics.minutes.map((value) => value.ram.temperature!.min)
											: $hardwareStatistics.seconds.map((value) => value.ram.temperature!.min),
									},
								],
								unit: " °C",
								time: minutes ? "m" : "s",

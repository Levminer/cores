					...
					{#snippet button()}
						...
						>
							...
							<div class="text-left">
								...
								<h3>Access your computer from your iPhone or Android device.</h3>
							</div>
						</Dialog.Trigger>
					{/snippet}
				</MobileDialog>
			...
			<div class="flex w-full flex-col gap-3 rounded-xl p-8 sm:p-4">
				...
				>
					...
					<div class="text-left">
						...
						<h3>You can see every component at a glance and customize the layout.</h3>
					</div>
				</button>
...
<script lang="ts">
	...
	const trial = () => {
		...
		// 	return takeStep("connections", true)
		// }
	...
	const activate = async () => {
		...
		if (!key) {
			return alert("Please enter a license key to activate Cores.")
		}

		if (key !== "") {
			const url = `https://act.coresmonitor.com?license_key=${key}&user_id=${data.user.id}`
			const options = {
				method: "GET",
			}
